...
export const getAllOutfits = async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM custom_item ORDER BY created_at DESC')
...
export const getOutfitById = async (req, res) => {
  try {
    const { id } = req.params
    const result = await pool.query('SELECT * FROM custom_item WHERE id = $1', [id])
...
export const createOutfit = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO custom_item (name, top, bottom, shoes, accessory, total_price)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
...
export const deleteOutfit = async (req, res) => {
  try {
    const { id } = req.params
    const result = await pool.query(
      'DELETE FROM custom_item WHERE id = $1 RETURNING id',
      [id]
    )
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Outfit not found' })
